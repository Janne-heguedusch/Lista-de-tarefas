  ...
  view.forEach((task) => {
    ...
    li.innerHTML = `
      ...
      <div class="title" contenteditable="false"
           title="Criada: ${created}\nAtualizada: ${updated}">
        ${escapeHTML(task.title)}
      </div>
      <div class="actions">
        <button class="icon-btn edit-btn" type="button">Editar</button>
        <button class="icon-btn delete-btn" type="button">Excluir</button>
      </div>
    `;
    list.appendChild(li);
  });
...
function showConfirm(li) {
  ...
  span.innerHTML = `
    <small>Excluir?</small>
    <button class="confirm-yes" type="button">Sim</button>
    <button class="confirm-no" type="button">Não</button>
  `;
  li.querySelector(".actions").appendChild(span);
}
